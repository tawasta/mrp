...
class MaterialRequirementLine(models.Model):
    ...
    bom = fields.Many2one(comodel_name="mrp.bom", string="BoM", readonly=False,)

    bom_lines = fields.One2many(
        comodel_name="mrp.bom.line", string="BoM lines", compute="_compute_bom_info",
    )

    can_be_manufactured = fields.Float(
        string="Manufacturable quantity", readonly=False,
    )
    ...
    material_requirement_id = fields.Many2one(
        "material.requirement", string="Material Requirement", readonly=False,
    )

    product_id = fields.Many2one(
        "product.product", string="Product", store=True, readonly=False,
    )

    product_internal_reference = fields.Char(related="product_id.default_code")

    product_availability = fields.Float(string="On Hand", readonly=False,)

    product_uom_id = fields.Char(
        related="product_id.uom_name", string="Product Unit of Measure",
    )

    promised_qty_line = fields.Char(string="Potential quantity", readonly=False,)

    qty_to_manufacture = fields.Float(string="Qty used in BOM", readonly=False,)

    variant = fields.Char(string="Variant", readonly=False,)

    vendor = fields.Char(
        string="Preferred supplier", compute="_compute_preferred_supplier",
    )
