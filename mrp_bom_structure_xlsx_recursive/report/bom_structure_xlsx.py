...
class ReportMrpBomStructureXlsxRecursiveStructure(models.AbstractModel):
    ...
    def product_material_summary(
        ...
        for ch in bom.bom_line_ids:
            ...
            materials_dict["materials_weight"] += sum(
                mater.net_weight * ch.product_qty * multiplier for mater in materials
            )
            ...
            materials_dict["wood_materials_weight"] += sum(
                mater.net_weight * ch.product_qty * multiplier
    ...
    def all_material_summary(
        ...
        for ch in bom.bom_line_ids:
            ...
            if product:
                products.append(
                    [product, ch.product_qty * multiplier, ch.product_uom_id]
                )
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for o in objects:
            ...
            for product, qty, uom in content_products:
                materials = self.env["product.material.composition"].search(
                    domain=[
            ...
            for material, weight_recyc in name_and_weight.items():
                ...
                sheet6.write(r, 11, (net_weight / total_grouped_net_weight) * 100)
            ...
            for product, qty, uom in pack_products:
                materials = self.env["product.material.composition"].search(
                    domain=[
                        ("product_product_id", "=", product.id),
                        ("type", "!=", "product_packaging"),
            ...
            for material, weight_recyc in name_and_weight.items():
                ...
                sheet6.write(r, 18, net_weight)
                sheet6.write(r, 19, net_weight * 0.001)  # weight in kg
                sheet6.write(r, 20, (net_weight / total_grouped_net_weight) * 100)
                check_weight += net_weight
                ...
                sheet6.write(
                    r, 25, (material.renewable_weight_percentage / 100) * net_weight
                )
                r += 1

            sheet6.write(r, 17, "Total")
            sheet6.write(r, 18, total_grouped_net_weight)
            sheet6.write(r, 19, total_grouped_net_weight * 0.001)
            sheet6.write(r, 20, (check_weight / total_grouped_net_weight) * 100)
            sheet6.write(
                r,
                21,
                total_grouped_net_weight
            ...
            sheet6.write(r, 23, total_grouped_biogenic_weight * 0.001)
            sheet6.write(
                r,
                24,
                total_grouped_net_weight
            ...
            sheet6.write(r, 25, total_grouped_renewable_weight)
            ...
            for consu_bom, oper_duration in bom_consus:
                ...
                for product, qty, uom in consu_products:
            ...
            for product, qty, uom in products:
                materials = self.env["product.material.composition"].search(
                    domain=[
                        ("product_product_id", "=", product.id),
                        ("type", "=", "product_packaging"),
