...
class SaleOrder(models.Model):
    ...
    def get_searchable_products(self, line):
        ...
        def get_sub_lines(current_bom, product_variant=False):
            ...
            for bom_line in current_bom.bom_line_ids:
                ...
                if bom_line.attribute_value_ids.ids and not all(
        ...
        bom = bom_model._bom_find(
            product_tmpl=line.product_id.product_tmpl_id, product=line.product_id
        )
    ...
    def product_mrp_area_create_multi_queued(self, line):
        ...
        return_text = (
            "Tried to create MRP Area Parameters from Sale Order line {}"
    ...
    def _product_mrp_area_create_multi_queued(self, lines):
        for line in lines:
            job_desc = "Create MRP Area Parameters from Sale Order line: {}".format(
                line.name
            )
            self.with_delay(description=job_desc).product_mrp_area_create_multi_queued(
                line
            )
