...
class ProductReport(models.Model):
    ...
    # mrp_move = fields.Many2one('mrp.move', readonly=True)
    ...
    def _select_product(self, fields=None, days=1):
        ...
        # mrm.id AS mrp_move,
        # SUM(mrm.mrp_qty) / NULLIF(COUNT(mrm), 0.0) AS move_sum,
        ...
        # SUM(mrm.mrp_qty) / NULLIF(count(DISTINCT svl), 0.0) AS move_sum,
        ...
        select_ = """
                ...
                ((sum(
                    svl.value * currency_table.rate) / NULLIF(sum(
                        svl.quantity), 0.0)) * stq.quantity) / NULLIF(
                        (SELECT COUNT(*) FROM mrp_move WHERE product_id = p.id
                        AND mrp_type='d'
                         AND mrp_date < CURRENT_DATE + INTERVAL '{} DAY'
                    UNION
                        SELECT COUNT(*) FROM mrp_move WHERE product_id = p.id
                        AND mrp_type='d'
                         AND mrp_date < CURRENT_DATE + INTERVAL '{} DAY'
                ...
                (stq.quantity - stq.reserved_quantity) / NULLIF(
                        (SELECT COUNT(*) FROM mrp_move WHERE product_id = p.id
                        AND mrp_type='d'
                         AND mrp_date < CURRENT_DATE + INTERVAL '{} DAY'
                    UNION
                ...
                sum(mrm.mrp_qty) /
                    NULLIF(
                    (SELECT COUNT(*) FROM stock_valuation_layer WHERE product_id = p.id UNION
                    SELECT COUNT(*) FROM stock_valuation_layer WHERE
                        product_id = p.id), 0.0) * -1
                    AS move_sum,
                ...
                (
                    ...
                    (
                        NULLIF(((stq.quantity - stq.reserved_quantity
                        + (SUM(mrm.mrp_qty) / NULLIF(
                            (SELECT COUNT(*) FROM stock_valuation_layer
                                WHERE product_id = p.id UNION
                            SELECT COUNT(*) FROM stock_valuation_layer
                                WHERE product_id = p.id), 0.0) * -1)
                        ) * prop.value_float), 0.0)
                    )
                )
                    * {} AS sufficiency,
                ...
                sum(mrm.mrp_qty) /
                    ...
                    SELECT COUNT(*) FROM stock_valuation_layer
                        WHERE product_id = p.id), 0.0) * -1
                    * prop.value_float AS cost_total,
    ...
    def _query(
        ...
        # with_clause = """svl_table (qty) as
        #    SELECT quantity FROM stock_valuation_layer WHERE product_id = p.id
        # """
