...
class MrpProduction(models.Model):
    ...
    def create_purchase_request(self):
        ...
        vals = {
            ...
            "origin": self.name,
            "description": "{} {}".format("Materials for", self.name),
        ...
        for material in self.move_raw_ids:
            if self.check_material_for_request(material):
                pr_line_vals = {
                    ...
                    "product_qty": self._get_material_qty_for_request(material),
                    "name": material.product_id.name,
                    "request_id": res.id,
                }

                if (
                    hasattr(self, "analytic_distribution")
                    and self.analytic_distribution
                ):
                    distr = dict()
                    distr[mo_account_id.id] = 100
                    pr_line_vals["analytic_distribution"] = distr
                purchase_request_line_model.create(pr_line_vals)

        return {
            "name": "%s / Purchase Request" % self.name,
            "view_type": "form",
            "view_mode": "form,tree",
